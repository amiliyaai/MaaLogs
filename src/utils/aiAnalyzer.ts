...
export function buildAnalysisPrompt(
  ...
  for (const node of failedNodes) {
    ...
    if (node.recognition_attempts && node.recognition_attempts.length > 0) {
      prompt += `- 识别尝试次数: ${node.recognition_attempts.length}\n`;
      prompt += `  识别尝试详情:\n`;
      for (const attempt of node.recognition_attempts) {
        const attemptStatus = attempt.status === "success" ? "成功" : "失败";
        prompt += `    - ${attempt.name}: ${attemptStatus}\n`;
        if (attempt.reco_details) {
          prompt += `      算法: ${attempt.reco_details.algorithm || "N/A"}\n`;
          prompt += `      结果: ${JSON.stringify(attempt.reco_details.detail || {})}\n`;
        }
      }
    }

    // 添加嵌套识别信息
    if (node.nested_recognition_in_action && node.nested_recognition_in_action.length > 0) {
      prompt += `- 动作中嵌套识别次数: ${node.nested_recognition_in_action.length}\n`;
      for (const nested of node.nested_recognition_in_action) {
        const nestedStatus = nested.status === "success" ? "成功" : "失败";
        prompt += `    - ${nested.name}: ${nestedStatus}\n`;
        if (nested.reco_details) {
          prompt += `      算法: ${nested.reco_details.algorithm || "N/A"}\n`;
        }
      }
    }
  }

...
export class TaskTreeDataProvider implements vscode.TreeDataProvider<TreeItem> {
  ...
  private getTaskItems(): TreeItem[] {
    ...
    return tasks.map((task) => {
      const statusIcon = task.status === "succeeded" ? "✓" : task.status === "failed" ? "✗" : "○";
      ...
      item.iconPath = new vscode.ThemeIcon(
        task.status === "succeeded" ? "check" : task.status === "failed" ? "error" : "loading~spin"
      );
      item.command = {
  ...
  private getNodeItems(taskKey: string): TreeItem[] {
    ...
    return task.nodes.map((node) => {
      ...
      const item = new TreeItem(`${statusIcon} ${node.name}`, vscode.TreeItemCollapsibleState.None);
      ...
      item.iconPath = new vscode.ThemeIcon(node.status === "success" ? "check" : "error");
      item.command = {
